...
impl Store {
    ...
    pub async fn quota(
        ...
        Ok(sqlx::query_as("WITH latest AS (SELECT DISTINCT ON (window_key) window_key,unit,remaining,maximum,observed_at_ms,valid_until_ms,resets_at_ms,source FROM quota_observations WHERE organization_id=$1 AND project_id=$2 AND account_id=$3 ORDER BY window_key,observed_at_ms DESC) SELECT latest.window_key,latest.unit,latest.remaining::text AS remaining,latest.maximum::text AS maximum,latest.observed_at_ms,latest.valid_until_ms,latest.resets_at_ms,latest.source,previous.remaining::text AS previous_remaining,previous.observed_at_ms AS previous_observed_at_ms,(latest.remaining IS NOT NULL AND latest.valid_until_ms > floor(extract(epoch FROM clock_timestamp())*1000)::bigint AND latest.resets_at_ms > floor(extract(epoch FROM clock_timestamp())*1000)::bigint) AS fresh FROM latest LEFT JOIN LATERAL (SELECT remaining,observed_at_ms,unit,maximum,resets_at_ms,source FROM quota_observations WHERE organization_id=$1 AND project_id=$2 AND account_id=$3 AND window_key=latest.window_key AND observed_at_ms<latest.observed_at_ms ORDER BY observed_at_ms DESC LIMIT 1) previous ON previous.unit=latest.unit AND previous.maximum IS NOT DISTINCT FROM latest.maximum AND previous.resets_at_ms=latest.resets_at_ms AND previous.source=latest.source ORDER BY latest.window_key")
