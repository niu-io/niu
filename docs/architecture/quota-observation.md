...
Collector credentials cover all accounts in their project. Per-account grants, a credential listing UI, rotation and actor-linked audit events remain open. The supplied source string still is not independently verified provider provenance. The JavaScript SDK provides `NiuCollectorClient`, bound to one project and exposing only quota ingestion. `NiuAdminClient` provides credential issuance and revocation for operator applications.
